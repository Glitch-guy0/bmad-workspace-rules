...
description: Guide for creating and evolving learned capabilities for Archiver
---
...
### Script
A bash script for deterministic tasks — file validation, index generation, format checking. Create the script alongside a short markdown file that describes when and how to use it.

```
capabilities/
├── {example-script}.md          # When to run, what to do with results
└── {example-script}.sh          # The actual computation
```
...
- Never hardcode paths — accept project root as argument or use relative paths
